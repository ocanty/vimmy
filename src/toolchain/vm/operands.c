...
[REGISTER] =                        { &vm_get_reg_dst, &vm_get_reg_src },
 ...
 // Only one immediate per instruction so it doesnt matter about dst/src
[CONSTANT] =                        { &vm_get_immediate, &vm_get_immediate },
...
// Only one displacement per instruction
[MEMORY_CONSTANT] =                 { &vm_get_memory_const, &vm_get_memory_const }
}; // indexed by operand type then by dst/src
...
[REGISTER] =						{ &vm_set_reg_dst, &vm_set_reg_src },
 ...
 // Cant set constants
[CONSTANT] =						{ &vm_set_void, &vm_set_void },
...
uint16_t vm_get_reg_dst(vm_state* vm) {
    return vm->m_Reg[vm->m_InstructionData.m_RegDst];
}
void vm_set_reg_dst(vm_state* vm, uint16_t val) {
    vm->m_Reg[vm->m_InstructionData.m_RegDst] = val;
}

// A
uint16_t vm_get_reg_src(vm_state* vm) {
    return vm->m_Reg[vm->m_InstructionData.m_RegSrc];
}
void vm_set_reg_src(vm_state* vm, uint16_t val) {
    vm->m_Reg[vm->m_InstructionData.m_RegSrc] = val;
}

// 0x100
uint16_t vm_get_immediate(vm_state* vm) {
    return vm->m_InstructionData.m_Immediate;
}

// [A]
uint16_t vm_get_memory_register_dst(vm_state* vm) {
    return vm_get_u16(vm, vm->m_Reg[vm->m_InstructionData.m_RegDst]);
}
void vm_set_memory_register_dst(vm_state* vm, uint16_t val) {
    vm_set_u16(vm, vm->m_Reg[vm->m_InstructionData.m_RegDst], val);
}

uint16_t vm_get_memory_register_src(vm_state* vm) {
    return vm_get_u16(vm, vm->m_Reg[vm->m_InstructionData.m_RegSrc]);
}
void vm_set_memory_register_src(vm_state* vm, uint16_t val) {
    vm_set_u16(vm, vm->m_Reg[vm->m_InstructionData.m_RegSrc], val);
}

// [A+0x100]
uint16_t vm_get_memory_registerdisp_dst(vm_state* vm) {
    return vm_get_u16(vm, 
        (uint16_t)vm->m_Reg[vm->m_InstructionData.m_RegDst] + 
            (int16_t)vm->m_InstructionData.m_Displacement
    );
}
void vm_set_memory_registerdisp_dst(vm_state* vm, uint16_t val) {
    vm_set_u16(vm,
        (uint16_t)vm->m_Reg[vm->m_InstructionData.m_RegDst] + 
            (int16_t)vm->m_InstructionData.m_Displacement, val
    );
}

uint16_t vm_get_memory_registerdisp_src(vm_state* vm) {
    return vm_get_u16(vm, 
        (uint16_t)vm->m_Reg[vm->m_InstructionData.m_RegSrc] + 
            (int16_t)vm->m_InstructionData.m_Displacement
    );
}
...
void vm_set_memory_const(vm_state* vm, uint16_t val) {
    vm_set_u16(vm, vm->m_InstructionData.m_Displacement, val);
}